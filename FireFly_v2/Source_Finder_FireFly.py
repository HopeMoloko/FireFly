...
import nest_functions_Firefly as nsf
...
if __name__ == "__main__":
 ...
 #   plt.pause(3)
    ...
    # mcmc stepsize
    param_step =  [.05,.02 , .08]  #[1.5, 2.5, 2.5][.2,.1,.3]
    ...
    logLikelihood, Prior, mcmc_steps, stepper, ndim, n_walkers, thresh
    )
...
#        "#------------------------------------------------------------------------------#"
    ...
    Prior, initial_theta, mcmc_firefly,thresh,stepsize,switch,samples_name)
    end = time.time()
    ...
    #Posterior results
    #np.savetxt(samples_name+'count_on.csv',counts_on,delimiter=',')
    #np.savetxt(samples_name+'count_off.csv',counts_off,delimiter=',')
