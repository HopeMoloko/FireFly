...
Data = pd.read_csv('example_data-Copy1.txt',delimiter=' ',names=['x','y','err'])
x_values = Data.x
y_values = Data.y
...
datafile = "chains-cubic-5000"
# run MultiNest
pymultinest.run(Log_Likelihood, prior_transform, n_params, outputfiles_basename=datafile , resume = False, verbose = True,n_live_points = 200,max_iter = 5000)
...
for (a, b, c) in am.get_equal_weighted_posterior()[::1000,:-1]:
	plt.plot(x_values, Model_function([a,b,c], x_values), '-', color='blue', alpha=0.3, label='data')
