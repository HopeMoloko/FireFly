...
def prior_transform_1(pixel_size,ndim, n_walkers):
    ...
    Returns : array
            A sample of each paramater drawn from the prior.
    """
    ...
    def switch_explorer(
        ...
        for index, source in enumerate(theta_out):
            ...
            if flux_old != 0.0 and state_new == 0.0:  # on to off
                ...
                if loglikelihood_trail > loglikelihood_constraint :
            ...
            if flux_old != 0.0 and state_new == 1.0:  # on to on
                ...
                theta_out = (theta_new)  # update old theta used for checking switch on-off in the loop
                loglikelihood_new = (loglikelihood_trail)  # update likelihood constraint to get the best sample
