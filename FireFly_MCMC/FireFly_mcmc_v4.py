...
def MH_mcmc(loglikelihood_func, Prior_func, theta, mcmc_steps, thresh, stepsize, switch, sample_file_name=None, checkpoint=True):
    ...
    if checkpoint == True:
        if os.path.exists('restore/' + sample_file_name + '-samples-restore.txt'): #check if restore file exists
            chain_sample_restored = np.loadtxt(('restore/' + sample_file_name + '-samples-restore.txt'), delimiter=',')
            # load saved loglikelihood values for each sample from unfinished run
            chain_logliklihood_restored = np.loadtxt(('restore/' + sample_file_name + '-loglikelihood-restore.txt'), delimiter=',') 
            ...
            theta = chain_sample[(-1)]
            naccept = int(stats['naccept'])
